# src/autoloads/GameManager.gd — Orchestrateur principal DOSCO
extends Node
...
func start_ai_game_online(galaxy_id: String, ai_level: String) -> void:
	start_ai_game(galaxy_id, ai_level)
...
func add_stars(amount: int) -> void:
	stars += amount; _save_season(); stars_updated.emit(stars)

func deduct_stars(amount: int) -> bool:
	if stars < amount: return false
	stars -= amount; _save_season(); stars_updated.emit(stars); return true

func finalize_game(winner: String, stake: int) -> void:
	var player_color: String = SaveManager.load_data().get("pending_game", {}).get("color", "W")
...
func _load_session() -> void:
	var s := AuthDB.get_session()
	if s.has("uid"):
		current_user = s; is_logged_in = true
		stars = s.get("stars", 100); wins = s.get("wins", 0); losses = s.get("losses", 0)

func _load_season() -> void:
	var s := SaveManager.load_season()
	if not s.is_empty():
		stars = s.get("stars", stars); wins = s.get("wins", wins)
		losses = s.get("losses", losses); win_streak = s.get("winStreak", 0)
